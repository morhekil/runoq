...
This guide takes you from a brand-new checkout of the `runoq` repository to successfully running both smoke test lanes (sandbox and lifecycle). It covers every prerequisite, the GitHub App creation, and the environment configuration needed before anything will work.
...
```bash
export RUNOQ_CLAUDE_BIN=/path/to/claude
export RUNOQ_SMOKE_CODEX_BIN=/path/to/codex
```

## 2. Clone The Runtime Repository

```bash
git clone <runoq-repo-url>
cd runoq
```
...
`runoq` authenticates as a GitHub App to manage issues, PRs, and labels on target repositories.
...
| Field | Value |
| --- | --- |
| **GitHub App name** | `runoqapp` (must match the `identity.appSlug` value in `config/runoq.json`) |
| **Homepage URL** | Any valid URL |
| **Webhook** | Uncheck **Active** — runoq does not use webhook delivery |

For **visibility**, choose:

- **Private / only on this account** if all repos that `runoq` will touch live under the same user or organization that owns the app. This is the preferred setup for a single-owner sandbox plus lifecycle eval.
...
```bash
mkdir -p "$HOME/.runoq"
mv ~/Downloads/<app-name>.*.private-key.pem "$HOME/.runoq/app-key.pem"
chmod 600 "$HOME/.runoq/app-key.pem"
```

Or place it anywhere and export the path:

```bash
export RUNOQ_APP_KEY="/path/to/your-key.pem"
```
...
- `app_slug` matches `identity.appSlug` in `config/runoq.json`
- `app_id` matches the App ID from step 4c

Do not use `gh api "/apps/<slug>"` as the primary check here. Private apps may return `404` on that endpoint even when the installation is correct, so `runoq init` resolves the App ID from the repository installation instead.
...
```bash
gh repo create <owner>/runoq-sandbox --private --clone
cd runoq-sandbox
...
```bash
gh api "repos/<owner>/runoq-sandbox/collaborators/<your-username>/permission" --jq '.permission'
...
export RUNOQ_SMOKE_REPO="<owner>/runoq-sandbox"
export RUNOQ_SMOKE_APP_ID="<numeric-app-id-from-step-4c>"
export RUNOQ_SMOKE_INSTALLATION_ID="<installation-id-from-step-4e>"
export RUNOQ_SMOKE_APP_KEY="$HOME/.runoq/app-key.pem"
export RUNOQ_SMOKE_PERMISSION_USER="<your-github-username>"
...
RUNOQ_SMOKE=1
RUNOQ_SMOKE_REPO=myorg/runoq-sandbox
RUNOQ_SMOKE_APP_ID=123456
RUNOQ_SMOKE_INSTALLATION_ID=78901234
RUNOQ_SMOKE_APP_KEY=/Users/me/.runoq/app-key.pem
RUNOQ_SMOKE_PERMISSION_USER=myusername
...
```bash
export RUNOQ_SMOKE_LIFECYCLE=1
export RUNOQ_SMOKE_REPO_OWNER="<owner-or-org>"
export RUNOQ_SMOKE_APP_ID="<numeric-app-id-from-step-4c>"
export RUNOQ_SMOKE_APP_KEY="$HOME/.runoq/app-key.pem"
```
...
```bash
export RUNOQ_SMOKE_REPO_PREFIX="runoq-live-eval"   # default
export RUNOQ_SMOKE_REPO_VISIBILITY="private"          # default
export RUNOQ_SMOKE_RUN_ID="my-run-001"                # auto-generated if omitted
```

**Important**: Lifecycle eval creates repos dynamically under `RUNOQ_SMOKE_REPO_OWNER`. Install the app on that same owner, and prefer **all repositories** access there. If you keep **only select repositories**, you must manually add each newly created lifecycle repo to the app before `runoq init` can succeed.
...
Preflight checks: `RUNOQ_SMOKE`, `RUNOQ_SMOKE_REPO_OWNER`, `RUNOQ_SMOKE_APP_ID`, `RUNOQ_SMOKE_APP_KEY`, operator `gh` auth, and that `claude`, `codex`, `node`, and `npm` are all on `PATH`.
...
```bash
ls .runoq/live-smoke/runs/<run_id>/
# init.log  run.log  state/  summary.json
```

## 10. Clean Up Lifecycle Repos

Managed repos are tracked in `.runoq/live-smoke/managed-repos.json`. Clean up explicitly:
...
- Confirm the app name matches `identity.appSlug` in `config/runoq.json` (default: `runoqapp`)
...
- Check the file exists at the path in `RUNOQ_SMOKE_APP_KEY` or `$HOME/.runoq/app-key.pem`
...
The sandbox script cleans up on success. If it fails mid-run, manually check the sandbox repo for leftover issues/PRs with `runoq` in the title and close them.
