...
`agendev init` creates `.agendev/identity.json`, `.agendev/state/`, any missing queue labels in GitHub, a default `package.json` when the target repo does not already have one, and a convenience `agendev` symlink.
