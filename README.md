...
- Uses `.runoq/state/*.json` as local recovery breadcrumbs rather than the source of truth for history.
...
- `bats`
- `shellcheck`
- Claude CLI, available on `PATH` as `claude` unless `RUNOQ_CLAUDE_BIN` is set
- A GitHub repository with an `origin` remote hosted on `github.com`
- A GitHub App private key at `$HOME/.runoq/app-key.pem` or a path supplied through `RUNOQ_APP_KEY`
...
`runoq init` creates `.runoq/identity.json`, `.runoq/state/`, any missing queue labels in GitHub, a default `package.json` when the target repo does not already have one, installs or refreshes runoq-managed `.claude/` agent and skill symlinks in the target repo, and creates a convenience `runoq` symlink.
