...
2. Reuse `"$AGENDEV_ROOT/templates/issue-template.md"` as the issue-body shape. Do not invent a parallel template in prompt text.
...
4. Flag bad granularity before creating issues:
   - Too broad: more than 5 acceptance criteria, multiple subsystems, or a plan that obviously needs further decomposition. See `"$AGENDEV_ROOT/test/fixtures/plans/broad-example.md"`.
   - Too narrow: trivial rename or formatting-only work with no behavioral impact. See `"$AGENDEV_ROOT/test/fixtures/plans/narrow-example.md"`.
   - Missing testability: no verifiable acceptance criteria or no observable outcome. See `"$AGENDEV_ROOT/test/fixtures/plans/untestable-example.md"`.
...
6. Only after confirmation, create issues through `"$AGENDEV_ROOT/scripts/gh-issue-queue.sh" create`. Do not hand-write `gh issue create` calls.
...
- Creation phase:
  - Use `"$AGENDEV_ROOT/scripts/gh-issue-queue.sh" create` for each approved issue.
  - Reuse `"$AGENDEV_ROOT/templates/issue-template.md"` language and section structure.
