...
func (a *App) runCreate(ctx context.Context, issue string, title string) int {
	...
	if err := a.execCommand(ctx, commandRequest{
		...
		return a.failf("Failed to fetch origin main: %v", err)
	}

	if _, err := os.Lstat(path); err == nil {
		return a.failf("Worktree already exists: %s", path)
	} else if !errors.Is(err, os.ErrNotExist) {
		return a.failf("Failed to inspect worktree path %s: %v", path, err)
	}
	...
	if err := a.execCommand(ctx, commandRequest{
		...
		return a.failf("Failed to create worktree: %v", err)
	}
...
func (a *App) runRemove(ctx context.Context, issue string) int {
	...
	if err := a.execCommand(ctx, commandRequest{
		...
		return a.failf("Failed to remove worktree: %v", err)
	}
...
func (a *App) runInspect(ctx context.Context, issue string) int {
	...
	if statErr != nil && !errors.Is(statErr, os.ErrNotExist) {
		return a.failf("Failed to inspect worktree path %s: %v", path, statErr)
	}
...
func (a *App) runBranchName(issue string, title string) int {
	...
	if err != nil {
		return a.failf("Failed to write branch name: %v", err)
	}
	return 0
}

func (a *App) targetRoot(ctx context.Context) (string, int) {
	if targetRoot, ok := envLookup(a.env, "TARGET_ROOT"); ok && targetRoot != "" {
		return targetRoot, 0
	}
...
func (a *App) appID(targetRoot string) string {
	if appID, ok := envLookup(a.env, "RUNOQ_APP_ID"); ok && appID != "" {
		return appID
	}
...
func defaultBaseRef(env []string) string {
	if baseRef, ok := envLookup(env, "RUNOQ_BASE_REF"); ok && baseRef != "" {
		return baseRef
	}
	return "origin/main"
}
...
func configPath(env []string, cwd string) string {
	if path, ok := envLookup(env, "RUNOQ_CONFIG"); ok && path != "" {
		return path
	}
	if root, ok := envLookup(env, "RUNOQ_ROOT"); ok && root != "" {
