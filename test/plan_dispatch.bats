...
@test "plan dispatch grants agents access to plan and milestone directories" {
  fixture_dir="$TEST_TMPDIR/fixtures"
  plan_dir="$TEST_TMPDIR/plan-dir"
  milestone_dir="$TEST_TMPDIR/milestone-dir"
  write_plan_dispatch_fixture "$fixture_dir"
  mkdir -p "$plan_dir" "$milestone_dir"
  cat >"$fixture_dir/task-decomposer.json" <<'EOF'
{"items":[
  {"key":"t1","type":"task","title":"Implement formatter","body":"## Acceptance Criteria\n\n- [ ] Works.","priority":1,"estimated_complexity":"low","complexity_rationale":"single module","depends_on_keys":[]}
],"warnings":[]}
...
EOF
  ...
  [ "$status" -eq 0 ]
  run grep -F -- "--add-dir $plan_dir" "$FAKE_CLAUDE_LOG"
  [ "$status" -eq 0 ]
  run grep -F -- "--add-dir $milestone_dir" "$FAKE_CLAUDE_LOG"
  [ "$status" -eq 0 ]
}
