...
@test "issue queue wrapper go fallback runs from RUNOQ_ROOT when runtime bin is unset" {
  ...
  fake_go_bin="$TEST_TMPDIR/fake-go-issue-queue"
  write_fake_go_bin "$fake_go_bin"

  run bash -lc 'cd "'"$project_dir"'" && RUNOQ_IMPLEMENTATION="" RUNOQ_ISSUE_QUEUE_IMPLEMENTATION="" RUNOQ_RUNTIME_BIN="" RUNOQ_GO_BIN="'"$fake_go_bin"'" "'"$RUNOQ_ROOT"'/scripts/gh-issue-queue.sh" list owner/repo runoq:ready'
  [ "$status" -eq 0 ]
  [[ "$output" == *"FAKE_GO_CWD:$RUNOQ_ROOT"* ]]
  [[ "$output" == *"FAKE_GO_ARGS:run $RUNOQ_ROOT/cmd/runoq-runtime __issue_queue list owner/repo runoq:ready"* ]]
}
