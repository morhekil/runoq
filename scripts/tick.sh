...
main() {
  ...
  if [[ -z "$current_epic" ]]; then
    if ! any_epic_exists "$issues_json"; then
      local epic_output planning_output planning_number
      ...
      planning_output="$(create_planning_issue "$issue_queue_script" "$repo" "$(printf '%s' "$epic_output" | jq -r '.url | capture("(?<n>[0-9]+)$").n')" "Break plan into milestones" "## Acceptance Criteria\n\n- [ ] Milestones proposed.")"
  ...
  if [[ -n "$approved_review" ]]; then
    local review_number review_body review_type review_parent review_view proposal_json selection_json filtered_json
    ...
    if [[ "$review_type" == "planning" ]]; then
      ...
      if [[ "$parent_title" == "Project Planning" ]]; then
        ...
        while IFS= read -r item; do
          [[ -n "$item" ]] || continue
          local title body priority complexity rationale depcsv
          title="$(printf '%s' "$item" | jq -r '.title')"
    ...
    if [[ "$review_type" == "adjustment" ]]; then
      local adjustment_json accepted_adjustments current_epic_json next_epic
      ...
      if [[ -n "$next_epic" ]]; then
        create_planning_issue "$issue_queue_script" "$repo" "$(printf '%s' "$next_epic" | jq -r '.number')" "Break down milestone into tasks" "## Acceptance Criteria\n\n- [ ] Tasks proposed." >/dev/null
      fi
      printf 'Applied approvals from #%s, created issues\n' "$review_number"
      exit 0
    fi
  ...
  if [[ -n "$planning_child" ]] && planning_issue_needs_dispatch "$planning_child" "$repo"; then
  ...
  if (( open_children == 0 )); then
    ...
    if [[ "$adjustment_count" -gt 0 ]]; then
      ...
      printf 'Milestone #%s review complete. Adjustments proposed on #%s\n' "$current_epic_number" "$(printf '%s' "$create_output" | jq -r '.url | capture("(?<n>[0-9]+)$").n')"
      exit 0
    fi
