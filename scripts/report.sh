...
summary_report() {
  ...
  jq -s '
    {
      issues: length,
      pass: map(select(.outcome.verdict == "PASS")) | length,
      fail: map(select(.phase == "FAILED" or .outcome.verdict == "FAIL")) | length,
      caveats: map(select(.outcome.verdict == "PASS_WITH_CAVEATS")) | length,
      tokens: {
      ...
      average_rounds: ((map(.rounds | length) | add // 0) / length)
    }
  ' $files
}
