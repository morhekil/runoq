...
identity_file() {
  printf '%s/.runoq/identity.json\n' "$(runoq::target_root)"
}

identity_json() {
  local file
  file="$(identity_file)"
  [[ -f "$file" ]] || runoq::die "No .runoq/identity.json found. Run 'runoq init' first."
  cat "$file"
}

resolve_key_path() {
  local key_path
  key_path="${RUNOQ_APP_KEY:-$(identity_json | jq -r '.privateKeyPath // empty')}"
  if [[ -z "$key_path" ]]; then
    runoq::die "No GitHub App private key configured. Run 'runoq init' first."
  fi
  key_path="${key_path/#\~/$HOME}"
  [[ -f "$key_path" ]] || runoq::die "GitHub App private key not found: $key_path"
  printf '%s\n' "$key_path"
}
...
export_token() {
  ...
  if [[ -n "${GH_TOKEN:-}" && -z "${RUNOQ_FORCE_REFRESH_TOKEN:-}" ]]; then
    printf 'export GH_TOKEN=%q\n' "$GH_TOKEN"
    return
  fi

  if [[ -n "${RUNOQ_TEST_GH_TOKEN:-}" ]]; then
    printf 'export GH_TOKEN=%q\n' "$RUNOQ_TEST_GH_TOKEN"
    return
  fi
  ...
  jwt="$(mint_jwt)"
  response="$(runoq::gh api \
    --method POST \
    "/app/installations/${installation_id}/access_tokens" \
    -H "Authorization: Bearer ${jwt}" \
    -H "Accept: application/vnd.github+json")"
  token="$(printf '%s' "$response" | jq -r '.token')"
  [[ -n "$token" && "$token" != "null" ]] || runoq::die "Failed to mint GitHub App installation token."
  printf 'export GH_TOKEN=%q\n' "$token"
}
