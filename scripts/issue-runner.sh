...
issue_runner_implementation="${RUNOQ_ISSUE_RUNNER_IMPLEMENTATION:-${RUNOQ_IMPLEMENTATION:-runtime}}"
case "$issue_runner_implementation" in
  ...
  runtime)
    ...
    if [[ -n "$runtime_bin" ]]; then
      exec "$runtime_bin" "__issue_runner" "$@"
    fi
    ...
    cd "$RUNOQ_ROOT"
    exec "$go_bin" run "$RUNOQ_ROOT/cmd/runoq-runtime" "__issue_runner" "$@"
    ;;
  *)
    echo "runoq: Unknown RUNOQ_ISSUE_RUNNER_IMPLEMENTATION: $issue_runner_implementation (expected shell or runtime)" >&2
    exit 1
    ;;
esac
